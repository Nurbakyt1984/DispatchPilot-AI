...
- po_number: the TQL/broker PO number, usually shown in the document header/title (e.g. "TQL PO# 37572042"). This is the load-level order number assigned by the broker.
- pu_number: the Pickup Number ("PU#"), found as its own labeled field or as a separate column in a PICKUPS table (e.g. "PU#" = "P9521"). This is NOT the same as po_number — it identifies the specific pickup/shed and is what the pickup location needs when the driver calls in. Do not copy po_number into pu_number. Use null if there is no separate PU#/Pickup Number value in the document.
...
def build_card(d: dict, merged: bool = False, fuel_price=None) -> str:
    ...
    card += "🚛 <b>LOAD SUMMARY</b>\n\n"

    # PU# показываем сверху карточки, отдельно от PO
    if d.get("pu_number"):
        card += f"📌 <b>PU#: {d.get('pu_number')}</b>\n"
