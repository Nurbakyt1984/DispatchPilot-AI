...
def build_card(d: dict) -> str:
    ...
    if pickup_city and delivery_city:
        url = maps_link(pickup_city, delivery_city)
        card += (
            f"\n🗺 <a href=\"{url}\">Open route in Google Maps</a>\n"
            f"\n📋 Tap to copy for Trucker Path:\n"
            f"<code>{pickup_city}</code>\n"
            f"<code>{delivery_city}</code>\n"
        )
