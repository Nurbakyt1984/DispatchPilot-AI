...
# ID своих групп через запятую — отчёт считает только по ним
MY_CHATS = [int(c.strip()) for c in os.getenv("MY_CHATS", "").split(",") if c.strip()]
...
def db():
    ...
    conn.execute("""CREATE TABLE IF NOT EXISTS chats(
        chat_id INTEGER PRIMARY KEY, title TEXT, last_seen TEXT)""")
    return conn


def record_chat(chat_id, title):
    """Запоминаем, в каких группах работает бот."""
    try:
        conn = db()
        conn.execute(
            """INSERT INTO chats(chat_id, title, last_seen) VALUES(?,?,?)
...
def week_stats(chat_ids=None):
    ...
    if chat_ids:
        ...
        row = conn.execute(
            f"""SELECT COUNT(*), COALESCE(SUM(miles),0),
                       COALESCE(SUM(rate),0), COALESCE(SUM(fuel),0)
                FROM loads WHERE ts >= ? AND ts < ?
                  AND chat_id IN ({marks})""",
            (prev_monday.isoformat(), this_monday.isoformat(), *chat_ids),
        ).fetchone()
    else:
        row = conn.execute(
...
def build_report():
    start, end, (count, miles, rate, fuel) = week_stats(MY_CHATS or None)
    net = rate - fuel
    avg = f"${rate / miles:.2f}" if miles else "—"
    scope = "<i>только мои группы</i>\n" if MY_CHATS else ""
    return (
...
def build_chats_report():
    """Список всех групп, где работает бот, + итоги каждой за неделю."""
    now = datetime.now(timezone.utc)
    this_monday = (now - timedelta(days=now.weekday())).replace(
        hour=0, minute=0, second=0, microsecond=0)
    prev_monday = this_monday - timedelta(days=7)
    conn = db()
    rows = conn.execute("SELECT chat_id, title FROM chats ORDER BY title").fetchall()
    out = [f"💬 <b>Группы бота</b> ({len(rows)})\n"]
    for cid, title in rows:
        st = conn.execute(
            """SELECT COUNT(*), COALESCE(SUM(miles),0), COALESCE(SUM(rate),0)
               FROM loads WHERE chat_id=? AND ts >= ? AND ts < ?""",
            (cid, prev_monday.isoformat(), this_monday.isoformat()),
        ).fetchone()
        mine = " ⭐" if cid in MY_CHATS else ""
        out.append(
            f"\n<b>{title}</b>{mine}\n"
            f"<code>{cid}</code>\n"
            f"за неделю: {st[0]} грузов · {st[1]:,.0f} mi · ${st[2]:,.0f}"
        )
    conn.close()
    if not rows:
        out.append("\nПока пусто — группы появятся после первого PDF.")
    return "\n".join(out)
...
Rules:
- is_rate_confirmation: true ONLY if this document is a Rate Confirmation, Load Confirmation, Carrier Confirmation, or pickup/delivery confirmation SENT BY THE BROKER. These are sent BEFORE delivery and authorize the load. Set false for: Bill of Lading (BOL) — the carrier document sent AFTER delivery showing what was actually hauled; invoices; lumper receipts; insurance certificates; blank forms; any document the carrier generates after pickup/delivery. BOL is NOT a rate confirmation even if it has PO, miles, or rate. If false, all other fields may be null.
- pickup_number: the pickup reference the driver gives at the shipper gate. Look for column headers or labels "PU#", "PU #", "PU", "Pickup #", "Pick Up #", "Reference number", "Ref #", "Confirmation #" near the pickup location block or inside the pickup table row. In TQL documents it is a column "PU#" in the pickup table (e.g. 802255341); in SME documents it appears as "Reference number: PU 3029-95". Return the value exactly as printed. Use null only if there is no such field anywhere.
...
def pdf_page_images(file_path: str, max_pages: int = 3):
    """Отрисовать страницы PDF в PNG (для сканов без текстового слоя)."""
    import base64
    images = []
...
async def parse_with_ai_images(images) -> dict:
    """Прочитать скан документа глазами Claude."""
    content = []
    for img in images:
        content.append({
            "type": "image",
    ...
    if "content" not in data:
        print(f"Anthropic vision error: {data}")
...
def build_card(d: dict, merged: bool = False, fuel_price=None) -> str:
    ...
    pu_line = f"📋 <b>PU#:</b> {d['pickup_number']}\n" if d.get("pickup_number") else ""
    card += (
        "🚛 <b>LOAD SUMMARY</b>\n\n"
...
async def download_pdf_text(update: Update) -> str:
    """Скачать PDF напрямую по ссылке Telegram — надёжно для больших файлов."""
    doc = update.message.document
    size_kb = (doc.file_size or 0) / 1024
    print(f"PDF received: {doc.file_name} ({size_kb:.0f} KB)")
    ...
    for attempt in range(3):
        ...
        try:
            file = await update.message.document.get_file(
                read_timeout=90, connect_timeout=30, pool_timeout=30)
            ...
            try:
                ...
                print(f"Direct download failed ({repr(direct_err)}), "
                      f"trying library method")

            if not ok:
                # Запасной путь: штатный метод библиотеки
                await file.download_to_drive(tmp_path, read_timeout=180,
                                             connect_timeout=30,
                                             pool_timeout=30)
            ...
            images = []
            if not text.strip():
                print("No text layer — rendering pages as images (scan)")
                images = pdf_page_images(tmp_path)
            os.remove(tmp_path)
            print(f"PDF parsed OK: {len(text)} chars, {len(images)} images")
            return text, images
        except Exception as e:
            last_err = e
            print(f"Download attempt {attempt + 1} failed: {repr(e)}")
...
async def process_text(update: Update, text: str, images=None):
    print(f"Processing PDF, {len(text)} chars, {len(images or [])} images")
    if not text.strip() and not images:
        return  # silently ignore unreadable PDFs

    try:
        if text.strip():
            data = await asyncio.wait_for(parse_with_ai(text), timeout=60)
        else:
            data = await asyncio.wait_for(parse_with_ai_images(images), timeout=90)
        ...
        if pickup_place and delivery_place:
            try:
                route_miles = await asyncio.wait_for(
                    real_route_miles(pickup_place, delivery_place), timeout=12)
            except Exception as e:
                print(f"Route skipped: {repr(e)}")
                route_miles = None
            if route_miles:
        ...
        try:
            fuel_price = await asyncio.wait_for(diesel_price(), timeout=8)
        except Exception as e:
            print(f"Fuel price skipped: {repr(e)}")
            fuel_price = None
        ...
        print(f"Card sent for PO {po}")
        record_load(chat_id, po, data, fuel_cost)
        try:
            record_chat(chat_id, update.message.chat.title or "Личка")
        except Exception:
            pass
    except Exception as e:
...
async def finish_album(group_key):
    ...
    if not album:
        return
    combined = "\n\n===== NEXT DOCUMENT =====\n\n".join(album["texts"])
    await process_text(album["update"], combined, album.get("images"))


async def pdf_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    doc = update.message.document
    print(f"PDF received: {doc.file_name} "
          f"({(doc.file_size or 0) / 1024:.0f} KB) chat={update.message.chat_id}")
    group_id = update.message.media_group_id

    if group_id:
        # Several PDFs sent as one message: collect them, answer once
        group_key = (update.message.chat_id, group_id)
        text, images = await download_pdf_text(update)
        ...
        if group_key not in ALBUMS:
            ...
            ALBUMS[group_key]["images"].extend(images)
            ALBUMS[group_key]["done_growing"] = False  # still growing, keep waiting
        return

    # Single PDF
    text, images = await download_pdf_text(update)
    await process_text(update, text, images)
...
async def chats_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Список групп, где работает бот — только владельцу."""
    if OWNER_ID and update.effective_user and update.effective_user.id == OWNER_ID:
        try:
            await context.bot.send_message(
                OWNER_ID, build_chats_report(), parse_mode="HTML")
        except Exception as e:
            print(f"Chats send error: {e}")
...
async def on_startup(app):
    asyncio.create_task(weekly_loop(app))
...
def main():
    ...
    app.add_handler(MessageHandler(
        filters.Document.PDF, pdf_handler, block=False))
